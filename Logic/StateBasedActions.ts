...
export const StateBasedActions = () =>
{
    ...
    AddChangeZoneGameAction("Removed",affected.map(a => a.id),1 as PlayerId, 'StateBasedActions 704.5d')
    ...
    AddChangeZoneGameAction("Removed",affected.map(a => a.id),1 as PlayerId, 'StateBasedActions 704.5e')

    //704.5f If a creature has toughness 0 or less, it’s put into its owner’s graveyard. Regeneration can’t replace this event.
    var affected = GetTribe("Creature").filter(c => c.zone === 'Field' && c.toughness <= 0)
    AddChangeZoneGameAction("Graveyard",affected.map(a => a.id),1 as PlayerId, 'StateBasedActions 704.5f')


    //704.5g If a creature has toughness greater than 0, it has damage marked on it, and the total damage marked on it is greater than or equal to its toughness, that creature has been dealt lethal damage and is destroyed. Regeneration can replace this event.
    var affected = GetTribe("Creature").filter(c => c.zone === 'Field' && c.toughness - c.damageMarked <= 0)
    AddChangeZoneGameAction("Graveyard",affected.map(a => a.id),1 as PlayerId, 'StateBasedActions 704.5g')
    ...
    */
    ...
    // Ascend: If a player controls ten or more permanents and doesn't have the city's blessing, they get the city's blessing
    GetPlayers().forEach(player => {
        const hasAscend = controller.cards.some(card => card.keywords.includes("Ascend") && card.controller === player.id && card.zone === 'Field');
        if (hasAscend)
        {
            const permanentsControlled = controller.cards.filter(card => card.controller === player.id && card.zone === 'Field').length;
            const hasCitysBlessing = controller.cards.some(card => card.name === "City's Blessing" && card.controller === player.id && card.zone === 'Field');
            
            if (permanentsControlled >= 10 && !hasCitysBlessing) {
                console.log(`Player ${player.id} gets the city's blessing (${permanentsControlled} permanents)`);
                NewCard("City's Blessing", player.id, 'Field', 1, newTimestamp());
            }
        }
